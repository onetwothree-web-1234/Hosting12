...
def init_db():
    with _db_lock, db() as conn:
        ...
        # single row, keeping the one with the most days_left remaining.
        ...
        dupe_groups = conn.execute(
            "SELECT uid, region FROM autolikes WHERE days_left > 0 "
            "GROUP BY uid, region HAVING COUNT(*) > 1"
        ).fetchall()
        for g in dupe_groups:
            dupe_rows = conn.execute(
                "SELECT id FROM autolikes WHERE uid=? AND region=? AND days_left > 0 "
                "ORDER BY days_left DESC, id DESC",
                (g["uid"], g["region"]),
            ).fetchall()
            remove_ids = [r["id"] for r in dupe_rows[1:]]  # keep the first (longest days_left), drop the rest
            if remove_ids:
...
def cmd_auto(message):
    ...
    with _db_lock, db() as conn:
        ...
        if existing:
            ...
            conn.execute(
                "UPDATE autolikes SET days_left=?, name=?, chat_id=?, added_by=? WHERE id=?",
                (days, name, message.chat.id, message.from_user.id, existing["id"]),
            )
        else:
            conn.execute(
    ...
    try:
        ...
        for repeat_num in range(1, repeat_count + 1):
            log.info("Auto-like batch %d/%d starting for %d UIDs x2 calls each, %d parallel workers",
                      repeat_num, repeat_count, len(rows), workers)
            to_delete, to_decrement = [], []

            with ThreadPoolExecutor(max_workers=workers) as executor:
                future_to_row = {executor.submit(_process_uid_double, row): row for row in rows}
                ...
                for fut in as_completed(future_to_row):
                    ...
                    new_days = row["days_left"] - (1 if repeat_num == repeat_count else 0)
...
def panel_auto_restart(message):
    ...
    if autolike_in_progress.is_set():
        ...
        return
    with db() as conn:
        pending = conn.execute("SELECT COUNT(*) c FROM autolikes WHERE days_left > 0").fetchone()["c"]
    if pending == 0:
