...
TEMPLATE_DEFS = {
    ...
    "tpl_group_only": {
        "label": "👥 Groups-Only Notice",
        "default": "🚫 This bot only works inside groups, not in private chat.\nAdd me to your group to use like/visit commands 👇",
        "placeholders": [],
        "highlight": set(),
    },
}
...
def raw_send_message(chat_id, text, entities=None, reply_markup=None):
...
def raw_send_photo(chat_id, photo_file_id, caption="", entities=None, reply_markup=None):
...
def raw_edit_message_text(chat_id, message_id, text, entities=None, reply_markup=None):
...
def send_rendered_template(chat_id, key, values, reply_markup=None, edit_message_id=None):
    ...
    formatting entities are never dropped."""
...
def verify_keyboard():
    ...
    kb.row(colored_button(button_text("btn_text_verify_check"), style="success",
                           icon_custom_emoji_id=button_icon("btn_text_verify_check"), callback_data="check_verify"))
    return kb
...
def add_to_group_kb():
    ...
    kb.add(colored_button(
        ...
        icon_custom_emoji_id=button_icon("btn_text_add_group"),
...
def group_only(message) -> bool:
    if message.chat.type not in ("group", "supergroup"):
        send_rendered_template(message.chat.id, "tpl_group_only", {}, reply_markup=add_to_group_kb())
        return False
    return True
...
def deny_owner_only(message):
    send_rendered_template(message.chat.id, "tpl_deny", {})
...
def send_flood_reply(message):
    send_rendered_template(message.chat.id, "tpl_flood", {})
...
def deliver_card(chat_id, message_id, card_key, values, default_text, keyboard):
    """Delivers the result card — the owner's fully custom template
    (with premium emoji / native formatting) if they've set one for
    card_key, otherwise falls back to the built-in styled box()."""
    if not has_custom_template(card_key):
        deliver_result(chat_id, message_id, default_text, keyboard)
        return
...
def extract_and_save_emojis(message):
    """Returns list of (label, emoji_id, display_text) newly saved."""
...
def cmd_like(message):
    ...
    if is_banned(message.from_user.id):
        try:
            ...
            pass
        send_rendered_template(message.chat.id, "tpl_ban", {})
        return
    ...
    if get_setting("maintenance", "0") == "1" and not is_owner(message.from_user.id):
        send_rendered_template(message.chat.id, "tpl_maintenance", {}, edit_message_id=processing.message_id)
        return
    ...
    if given == 0:
        deliver_card(message.chat.id, processing.message_id, "tpl_like_card", tpl_values, text, add_to_group_kb())
...
def cmd_visit(message):
    ...
    deliver_card(message.chat.id, processing.message_id, "tpl_visit_card", visit_tpl_values, text, add_to_group_kb())
    send_rendered_template(message.chat.id, "tpl_visit_success", visit_tpl_values)
...
def owner_flow_handler(message):
    ...
    if action == "add_emoji_mode":
        ...
        if message.content_type != "text":
            ...
            return
        saved = extract_and_save_emojis(message)
        if saved:
            confirm_saved_emojis(message.chat.id, saved)
        else:
            bot.reply_to(message, "⚠️ No new custom emoji found in that message. Send one, or type 'done'.")
        return
