...
if __name__ == "__main__":
    ...
    # when a second process starts polling with the same token (exactly
    # what happens for a few seconds/minutes during a Railway redeploy
    # while the old container is still shutting down). That meant a
    # "loser" duplicate process could never actually detect it had lost
    # and exit — it just sat there forever, still running its own
    # scheduler in the background, still firing backup_db_job on its own
    # 30-minute timer against its own (often fresh/near-empty) database.
    # Using bot.polling() directly instead lets a Conflict actually reach
    # this except block, so a duplicate instance can shut its scheduler
    # down and exit the process instead of lingering as a zombie.
        ...
        try:
            ...
            if getattr(e, "error_code", None) == 409:
                ...
                log.error(
                    ...
                    "already polling with this same token. Assuming this process "
                    "is the duplicate/loser — shutting its scheduler down and "
                    "exiting instead of retrying forever, so it can't keep "
                    "silently overwriting the real backup with its own stale "
                    "database. If this keeps happening on every deploy, check "
                    "Railway for more than one running instance/replica.",
