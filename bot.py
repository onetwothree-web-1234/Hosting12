...
def autolike_job():
    ...
    into one result per UID via _merge_double_like. While this runs,
    ...
    try:
        ...
        log.info("Auto-like job starting for %d UIDs x2 calls each, %d parallel workers",
                  len(rows), workers)
        to_delete, to_decrement = [], []
        rows_by_id = {row["id"]: row for row in rows}
        pending = {row["id"]: [] for row in rows}  # row_id -> [(data, elapsed, err), ...]

        with ThreadPoolExecutor(max_workers=workers) as executor:
            fut_to_rowid = {}
            for row in rows:
                for _ in range(2):  # "double" — 2 like-API calls per UID, run concurrently
            ...
            for fut in as_completed(fut_to_rowid):
                ...
                if len(pending[row_id]) < 2:
                    continue  # still waiting on this UID's other call
...
# ------------------------------------------------------------------
_PANEL_BUTTON_TEXTS = {
    BTN_STATS, BTN_AUTOLIST, BTN_RM_AUTO, BTN_SET_TIME, BTN_SET_GAP, BTN_SET_WORKERS, BTN_BAN, BTN_UNBAN, BTN_RESTRICT_UID,
