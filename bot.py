...
def verify_keyboard():
    """Channels join buttons + check button with emoji icons."""
    channels = get_verification_channels()
    rows = []
    ...
    if channels:
        # First channel takes full row
        rows.append([ch_button(channels[0])])
        # Next channels: 2 per row if available
        mid = channels[1:3]
        if mid:
            rows.append([ch_button(c) for c in mid])
        # Remaining channels: 2 per row
        remaining = channels[3:]
        for i in range(0, len(remaining), 2):
            rows.append([ch_button(c) for c in remaining[i:i+2]])

    # Check button with hardcoded emoji
...
# ------------------------------------------------------------------
def join_group_kb():
    """Join button with hardcoded emoji icon (red color as requested)."""
    btn = raw_button(
...
def cmd_start(message):
    ...
    # Check if welcome messages are disabled for this group
    if message.chat.type in ("group", "supergroup") and not is_welcome_enabled(message.chat.id):
        return
...
def _merge_double_like(results):
    """results: up to two (data, elapsed, err) tuples — the two
    concurrent like-API calls made for one UID. Many of these unofficial
    like APIs accept more than one call per run (up to the target
    account's own daily received-likes cap), so calling twice per UID
    can add more likes than calling once. Merge into a single
    representative result: total likes given across both calls, the
    earliest genuine "before" count, and the latest "after" count."""
    oks = [r for r in results if r[0] is not None]
    if not oks:
        first_err = results[0][2] if results else None
        return None, None, first_err
    total_given = sum((d.get("LikesGivenByAPI", 0) or 0) for d, _, _ in oks)
    best = max(oks, key=lambda r: (r[0].get("LikesafterCommand", 0) or 0))
    data = dict(best[0])
    data["LikesGivenByAPI"] = total_given
    if len(oks) == 2:
        data["LikesbeforeCommand"] = min(
            (oks[0][0].get("LikesbeforeCommand", 0) or 0),
            (oks[1][0].get("LikesbeforeCommand", 0) or 0),
        )
    elapsed = max((e for _, e, _ in oks if e is not None), default=None)
...
def autolike_job():
    ...
    time. EACH UID gets TWO concurrent like-API calls ("double"), merged
    into one result per UID via _merge_double_like. The batch can repeat
    multiple times (owner-editable "autolike_repeat_count" setting, default 1).
    While this runs, /like and /visit commands are queued (see _queue_during_autolike)
    ...
    try:
        ...
        for repeat_num in range(1, repeat_count + 1):
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for row in rows:
                    for _ in range(2):
                        fut = executor.submit(call_like_api, row["uid"], row["region"])
                        fut_to_rowid[fut] = row["id"]

                for fut in as_completed(fut_to_rowid):
                    row_id = fut_to_rowid[fut]
                    try:
                        data, elapsed = fut.result()
                        pending[row_id].append((data, elapsed, None))
                    except Exception as e:  # noqa: BLE001
                        pending[row_id].append((None, None, e))

                    if len(pending[row_id]) < 2:
                        continue

                    row = rows_by_id[row_id]
                    data, elapsed, err = _merge_double_like(pending.pop(row_id))
                    chat_id = row["chat_id"]
                    result_record = {
                        "uid": row["uid"],
                        "name": row["name"],
...
def panel_auto_restart(message):
    """Manually restart auto-like batch immediately."""
    try:
        autolike_job()
        bot.send_message(message.chat.id, "🔄 Auto-like batch restarted and running now!")
    except Exception as e:  # noqa: BLE001
        bot.send_message(message.chat.id, f"❌ Auto-like restart failed: {e}")
