...
BOT_THREADS = int(os.environ.get("BOT_THREADS", "300"))
...
_adapter = requests.adapters.HTTPAdapter(pool_connections=300, pool_maxsize=300, max_retries=0)
...
_verify_executor = ThreadPoolExecutor(max_workers=20)
_verify_cache = {}
...
def cmd_like(message):
    ...
    if restriction and restriction["block_like"]:
        bot.reply_to(message, "🔒 Likes are currently disabled for this UID.")
        return
    ...
    if given == 0:
        ...
        with _db_lock, db() as conn:
            ...
            conn.commit()
        text += "\n\n<b>" + fullwidth("🎉 Like sent successfully!") + "</b>"
...
def cmd_visit(message):
    ...
    if restriction and restriction["block_visit"]:
        bot.reply_to(message, "🔒 Visits are currently disabled for this UID.")
        return
    ...
    text += "\n\n<b>" + fullwidth("🎉 Visit sent successfully!") + "</b>"
...
_PANEL_BUTTON_TEXTS = {
    ...
    BTN_SET_BAN_MSG, BTN_VERIFY_CH, BTN_BACKUP_NOW, BTN_RESTORE_UP, BTN_CLOSE,
}
