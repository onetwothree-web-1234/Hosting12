...
OWNER_IDS = _parse_owner_ids(os.environ.get("OWNER_IDS", ""))
if not OWNER_IDS:
    log.warning("OWNER_IDS is empty — admin/owner-only commands (broadcast, panel, "
                "backup) will not work for anyone until you set it.")
else:
...
# API CALLS
# ------------------------------------------------------------------
def call_like_api(uid: str, region: str, timeout=20):
...
def call_visit_api(uid: str, region: str, timeout=20):
...
def parse_region_uid(args):
    if len(args) != 2:
        return None, None, "⚠️ Usage: <code>/like IND 1234567890</code>"
...
def cmd_auto(message):
    ...
    if not is_group_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, "🚫 Only group admins can set auto-like.")
        return
...
def cmd_unauto(message):
    ...
    if not is_group_admin(message.chat.id, message.from_user.id):
        bot.reply_to(message, "🚫 Only group admins can remove auto-like.")
        return
