...
# The owner just sends any message containing a Telegram custom/premium
# emoji to the bot (DM) — every custom_emoji entity found gets captured,
# deduplicated, and saved automatically. No manual ID entry needed.
# ------------------------------------------------------------------
@bot.message_handler(
    func=lambda m: m.chat.type == "private" and is_owner(m.from_user.id) and m.content_type == "text"
                   and any(e.type == "custom_emoji" for e in (m.entities or [])),
    content_types=["text"],
)
def auto_save_custom_emoji(message):
    saved = []
    with _db_lock, db() as conn:
    ...
    for label, emoji_id, display_text in saved:
        ...
        try:
            raw_send_message(message.chat.id, conf_text, entities=conf_entities)
        except Exception as e:  # noqa: BLE001
...
def panel_saved_emojis(message):
    ...
    admin_state[message.from_user.id] = {"action": "manage_emojis"}

    if not rows:
        bot.send_message(
            message.chat.id,
            "😀 No custom emoji saved yet — just send me one in DM (any message containing "
            "a Telegram Premium/custom emoji) and I'll save it automatically.",
            reply_markup=back_only_kb(),
        )
        return
    ...
    try:
        raw_send_message(message.chat.id, catalog_text, entities=entities)
    except Exception as e:  # noqa: BLE001
...
_PANEL_BUTTON_TEXTS = {
    ...
    BTN_SET_BAN_MSG, BTN_VERIFY_CH, BTN_CHANNEL_LIST, BTN_EDIT_MSGS, BTN_EDIT_BTNS, BTN_SAVED_EMOJIS,
