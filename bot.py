...
AUTOLIKE_WORKERS = 250  # thousands of UIDs run the batch, so this needs real concurrency
DEFAULT_AUTOLIKE_REPEAT = 1  # how many times to run the whole batch each day (each repeat = 2 calls per UID)
...
BOT_THREADS = int(os.environ.get("BOT_THREADS", "200"))
...
# A fresh sqlite3.connect() on every single db() call — which is how
# this used to work — costs real time (file-open overhead) on every
# command, multiplied across thousands of users, and the old
# `with db() as conn:` pattern never actually closed that connection
# afterward either (sqlite3's own context manager only wraps a
# transaction, not the connection lifetime) — so connections piled up
# between garbage-collection cycles. A small reusable pool fixes both:
# connections are opened once and handed back after use instead of
# opened-and-leaked per call. SQLite in WAL mode (set once below)
# supports many concurrent readers fine, and every writer already goes
# through the single _db_lock, so pooling several connections is safe.
_DB_POOL_SIZE = 12
_db_pool = queue.Queue(maxsize=_DB_POOL_SIZE)
_db_pool_lock = threading.Lock()
_db_pool_created = 0


def _reset_db_pool():
    """Drain and close every pooled connection and reset the pool's
    counter, so the NEXT db() call opens a brand-new connection against
    whatever is currently on disk at DB_PATH.

    This is required after swapping the database file out from under
    the process (restore-on-startup, restore-from-uploaded-file). A
    pooled sqlite3.Connection keeps its file handle open on the file it
    originally opened. os.replace()/overwriting the file afterwards
    does NOT make that connection see the new content — on Linux the
    old (now unlinked) inode just stays alive as long as the handle is
    open, so every query going through a stale pooled connection kept
    silently reading/writing a ghost copy of the OLD database that no
    longer exists. That's why restores looked like they "didn't take":
    the file on disk was correct, but the live bot never used it."""
    ...
    try:
        ...
        with _db_pool_lock:
            if _db_pool_created < _DB_POOL_SIZE:
                ...
                conn = None
        if conn is None:
            conn = _db_pool.get()  # pool is fully checked out — wait for one to free up
    try:
        with conn:  # sqlite3's own context manager: commit on success, rollback on exception
            yield conn
    finally:
        try:
            _db_pool.put_nowait(conn)
        except queue.Full:  # pool briefly oversized (shouldn't normally happen) — just close it
            conn.close()
    ...
    with _db_lock, db() as conn:
        ...
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS service_status (
                service TEXT PRIMARY KEY, enabled INTEGER DEFAULT 1
            );
            CREATE TABLE IF NOT EXISTS group_settings (
                chat_id INTEGER PRIMARY KEY, welcome_enabled INTEGER DEFAULT 1
            );
            """
        )
        conn.commit()
        ...
        defaults = {
            ...
            "autolike_workers": str(AUTOLIKE_WORKERS),
            "autolike_repeat_count": str(DEFAULT_AUTOLIKE_REPEAT),
            "like_reset_hour": "3",
        ...
        # Initialize service status (like, visit enabled by default)
        for service in ("like", "visit"):
            conn.execute("INSERT OR IGNORE INTO service_status (service, enabled) VALUES (?,?)", (service, 1))

        # Migration: autolikes originally only supported a day-countdown.
        # Add columns for the "likes target" mode (e.g. "/auto IND UID 999
        # srking" with no trailing "d" — completes once that many likes
        # have actually been given, instead of after N days) without
        # breaking any existing day-based rows, which all default to
        # mode='days' and keep working exactly as before.
        existing_cols = {r["name"] for r in conn.execute("PRAGMA table_info(autolikes)").fetchall()}
        if "mode" not in existing_cols:
            conn.execute("ALTER TABLE autolikes ADD COLUMN mode TEXT NOT NULL DEFAULT 'days'")
        if "target_likes" not in existing_cols:
            conn.execute("ALTER TABLE autolikes ADD COLUMN target_likes INTEGER NOT NULL DEFAULT 0")
        if "likes_given_total" not in existing_cols:
            conn.execute("ALTER TABLE autolikes ADD COLUMN likes_given_total INTEGER NOT NULL DEFAULT 0")

        # One-time cleanup: collapse duplicate active autolike rows for the
        # same uid+region (from before /auto de-duped on insert) into a
        # single row, keeping the one with the most work remaining.
        # Duplicate rows were making the exact same account get processed
        # — and reported — more than once per batch.
        dupe_groups = conn.execute(
            f"SELECT uid, region FROM autolikes WHERE {AUTOLIKE_ACTIVE_SQL} "
            "GROUP BY uid, region HAVING COUNT(*) > 1"
        ).fetchall()
        for g in dupe_groups:
            dupe_rows = conn.execute(
                f"SELECT id FROM autolikes WHERE uid=? AND region=? AND {AUTOLIKE_ACTIVE_SQL} "
                "ORDER BY (CASE mode WHEN 'days' THEN days_left ELSE (target_likes - likes_given_total) END) DESC, id DESC",
                (g["uid"], g["region"]),
            ).fetchall()
            remove_ids = [r["id"] for r in dupe_rows[1:]]  # keep the first (most remaining), drop the rest
            if remove_ids:
                conn.executemany("DELETE FROM autolikes WHERE id=?", [(i,) for i in remove_ids])
                log.info("Removed %d duplicate auto-like row(s) for UID %s (%s)",
                          len(remove_ids), g["uid"], g["region"])
...
def is_service_enabled(service: str) -> bool:
    """Check if a service (like/visit) is enabled globally."""
    with db() as conn:
        row = conn.execute("SELECT enabled FROM service_status WHERE service=?", (service,)).fetchone()
    return (row["enabled"] if row else 1) == 1


def set_service_status(service: str, enabled: bool):
    """Enable or disable a service globally."""
    with _db_lock, db() as conn:
        conn.execute(
            "INSERT INTO service_status (service, enabled) VALUES (?,?) "
            "ON CONFLICT(service) DO UPDATE SET enabled=excluded.enabled",
            (service, 1 if enabled else 0),
        )
        conn.commit()


def is_welcome_enabled(chat_id: int) -> bool:
    """Check if welcome message is enabled for a group."""
    with db() as conn:
        row = conn.execute("SELECT welcome_enabled FROM group_settings WHERE chat_id=?", (chat_id,)).fetchone()
    return (row["welcome_enabled"] if row else 1) == 1


def set_welcome_status(chat_id: int, enabled: bool):
    """Enable or disable welcome messages for a group."""
    with _db_lock, db() as conn:
        conn.execute(
...
def _maybe_trigger_autolike(reason):
    ...
    The "already ran today" flag is ALSO persisted to the settings table,
    not just kept in the _autolike_last_run_date module variable — a
    bare in-memory flag forgets everything the instant the process
    restarts (a Railway redeploy, a crash-and-relaunch, anything), so a
    threshold-trigger earlier in the day followed by a restart before
    the scheduled cron time would let the cron fire a SECOND full batch
    that same day, with no memory of the first one. This was very
    likely the actual cause of auto-like appearing to run twice even
    with the repeat count set to 1."""
    global _autolike_last_run_date
    today = usage_day()
    with _autolike_trigger_lock:
        if _autolike_last_run_date == today:
            return False
        if get_setting("autolike_last_run_date", "") == today:
            _autolike_last_run_date = today  # sync memory to what the DB already knows
            return False
        _autolike_last_run_date = today
        set_setting("autolike_last_run_date", today)
...
def restore_db_from_backup():
    """Pulls the pinned backup file from the backup channel and makes it
    the live database. Returns a short status string (used in the
    "latest deploy detected" notice sent to owners on startup).

    Must run BEFORE init_db()/any db() call ever opens a connection —
    see _reset_db_pool()'s docstring for why a stale open connection
    silently defeats a restore. It also wipes any leftover -wal/-shm
    sidecar files before writing the restored copy in: those files hold
    not-yet-checkpointed pages for whatever database used to be at
    DB_PATH, and SQLite will try to replay them against the new file on
    next open — which is exactly the kind of corruption that used to
    make "restore" quietly not work."""
    ...
    try:
        ...
        if not pinned or not pinned.document:
            ...
            return "No pinned backup found — starting fresh."
        ...
        return "✅ Database restored from the backup channel."
    except Exception as e:  # noqa: BLE001
        log.warning("Could not restore backup (probably first run): %s", e)
        return f"⚠️ Restore skipped/failed: {e}"


def _live_db_row_counts():
    """Snapshot of the live DB's important tables, used to sanity-check
    a backup before it overwrites the good pinned one in the channel."""
    try:
        conn = sqlite3.connect(DB_PATH)
        counts = {t: conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0]
                  for t in ("users", "groups", "channels", "custom_emojis")}
        conn.close()
        return counts
    except Exception:  # noqa: BLE001
        return None
...
def backup_db_job():
    ...
    # Guard against silently overwriting a good backup with a near-empty
    # one. This is exactly what happens if two bot processes are briefly
    # alive at once with the same token (e.g. a Railway redeploy where the
    # old container hasn't fully stopped yet) — the "loser" process keeps
    # its own scheduler running against its own fresh/near-empty database
    # and, every 30 minutes, quietly pins that over the real backup. A
    # legitimate backup should never suddenly lose most of its users.
    counts = _live_db_row_counts()
    if counts is not None:
        try:
            prev = json.loads(get_setting("last_backup_row_counts", "") or "{}")
        except Exception:  # noqa: BLE001
            prev = {}
        prev_users = prev.get("users", 0)
        now_users = counts.get("users", 0)
        if prev_users >= 10 and now_users < prev_users * 0.5:
            alert = (
                f"🛑 <b>Backup SKIPPED</b> — this instance's live database looks "
                f"suspiciously empty (users {prev_users} → {now_users}). NOT "
                f"overwriting the good pinned backup.\n\n"
                f"This almost always means two copies of the bot are running at "
                f"once with the same token (e.g. an old and new deploy overlapping). "
                f"Check Railway for duplicate/zombie instances before doing anything else."
            )
            log.error(alert)
            for oid in OWNER_IDS:
                try:
                    bot.send_message(oid, alert)
                except Exception:  # noqa: BLE001
                    pass
            return False, alert
    ...
    try:
        ...
        if counts is not None:
            set_setting("last_backup_row_counts", json.dumps(counts))
...
_verify_executor = ThreadPoolExecutor(max_workers=300)
...
VERIFY_CACHE_TTL = 600  # 10 minutes — already-verified users skip re-check entirely

# Channels list cache — get_verification_channels() does a DB query that
# is called on EVERY /like and /visit command (inside is_verified). With
# hundreds of simultaneous users, that's hundreds of DB hits per second
# for a list that almost never changes. Cache it in memory and refresh
# once per minute at most.
_channels_list_cache = None
_channels_list_cache_time = 0.0
_channels_list_cache_lock = threading.Lock()
CHANNELS_LIST_CACHE_TTL = 60  # seconds — invalidated instantly by add/remove channel ops


def get_verification_channels_cached():
    """In-memory cache over get_verification_channels(). Refreshes from
    DB at most once per minute. Callers that just changed the channel
    list (add/remove ops) call invalidate_channels_cache() immediately
    so the very next lookup picks up the new list."""
    global _channels_list_cache, _channels_list_cache_time
    with _channels_list_cache_lock:
        if (_channels_list_cache is not None
                and time.time() - _channels_list_cache_time < CHANNELS_LIST_CACHE_TTL):
            return _channels_list_cache
    fresh = get_verification_channels()
    with _channels_list_cache_lock:
        _channels_list_cache = fresh
        _channels_list_cache_time = time.time()
    return fresh


def invalidate_channels_cache():
    """Call this immediately after adding or removing a verification
    channel so the next is_verified() call picks up the new list."""
    global _channels_list_cache, _channels_list_cache_time
    with _channels_list_cache_lock:
        _channels_list_cache = None
        _channels_list_cache_time = 0.0
...
def _check_one_channel(ch, user_id):
    """Returns ch['name'] if the user is NOT a member of this channel,
    else None.

    NO retries, NO sleep. The old retry-with-0.4s-sleep design was the
    single biggest cause of verification slowness: with 40 channels and
    a transient Telegram hiccup, every channel that errored added 400ms
    of dead sleep-time. Since all 40 checks run in parallel (each on its
    own worker in _verify_executor, which now has 300 workers), the
    total verification time equals the SLOWEST single channel, not their
    sum. A single failed check returning "not a member" is safer than
    sitting idle for 400ms — the user just taps the verify button again.
    Errors are still logged so a misconfigured channel shows up clearly."""
    try:
        ...
        log.warning(
            "VERIFY CHECK failed for channel '%s' (%s) on user %s: %s — "
            "treating as NOT joined. If this keeps happening, the bot is likely "
            "not an admin in that channel (or the username changed).",
            ch["name"], ch["username"], user_id, e,
        )
        return ch["name"]
...
def is_verified(user_id: int, use_cache=True):
    ...
    Checks ALL channels SIMULTANEOUSLY — each channel gets its own
    dedicated worker in _verify_executor (300 workers, so even 40+
    channels all fire at the same instant with zero queuing). Total
    verification time = time of the slowest single Telegram API call,
    typically 100-300ms, not N × that.

    Positive results are cached for VERIFY_CACHE_TTL seconds so repeat
    commands from an already-verified user never hit Telegram at all."""
    if use_cache:
    ...
    channels = get_verification_channels_cached()
    # Submit all checks at once — with 300 workers they all start immediately
    futures = [_verify_executor.submit(_check_one_channel, ch, user_id) for ch in channels]
    missing = [f.result() for f in futures]
...
def colored_button(text, style=None, icon_custom_emoji_id=None, **kwargs):
    ...
    an optional premium custom-emoji icon before the label. Both `style`
    and `icon_custom_emoji_id` are Bot API 9.4 fields for INLINE buttons
    only — a plain reply-keyboard KeyboardButton (used for the whole
    admin panel, including the "🔙 Back" button) doesn't support them,
    and sending one that carries them can make Telegram reject the
    keyboard outright, which is exactly the kind of thing that makes a
    Back button look like it "does nothing" when tapped. So: only ever
    attach them when we're actually building an inline button."""
    ...
    if style and is_inline:
        build_kwargs["style"] = style
    if icon_custom_emoji_id and is_inline:
...
def verify_keyboard():
    """Channel-join buttons, 2 per row from the very first one (so 5
    channels = row of 2, row of 2, row of 1 — never a lone big button
    up top), each with the join-button emoji icon, red/danger style.
    The final check button gets its own emoji icon, green/success style."""
    channels = get_verification_channels_cached()
    rows = []
    ...
    def ch_button(ch):
        ...
        name = ch["name"]
        if len(name) > 24:  # keep long channel names from stretching the button oversized
            name = name[:23] + "…"
        return raw_button(
            name,
            style="danger",  # red, as requested
            icon_custom_emoji_id="6129527246085429728",  # join-button emoji
            url=link,
        )

    for i in range(0, len(channels), 2):
        rows.append([ch_button(c) for c in channels[i:i + 2]])

    rows.append([raw_button(
        button_text("btn_text_verify_check"),
        style="success",  # green
        icon_custom_emoji_id="6156558816763714393",  # verify-button emoji
...
# ------------------------------------------------------------------
def join_group_kb():
    """The 'this bot only works in our main group' redirect button —
    shown when the bot is used somewhere else. Red, with its own
    dedicated emoji icon (distinct from the per-channel verify buttons)."""
    btn = raw_button(
        button_text("btn_text_join_group"),
        style="danger",  # red color
        icon_custom_emoji_id="6129652186684070216",  # "join our group" redirect-button emoji
        url=MAIN_GROUP_LINK,
    )
...
def get_owner_id():
    """Return the first (usually only) owner ID for sending notifications."""
    return next(iter(OWNER_IDS), None) if OWNER_IDS else None
...
def extract_and_save_emojis(message):
    ...
    Also repairs old rows that were saved with a missing/blank
    display_text (from an earlier, broken save) — those rows silently
    never matched anything in get_emoji_auto_map(), which is why some
    already-saved premium emoji never auto-applied anywhere even though
    the ID itself was correctly stored."""
    ...
    with _db_lock, db() as conn:
        for e in message.entities:
            ...
            existing = conn.execute(
                "SELECT display_text FROM custom_emojis WHERE custom_emoji_id=?", (emoji_id,)
            ).fetchone()
            if existing:
                if not existing["display_text"] and display_text:
                    conn.execute(
                        "UPDATE custom_emojis SET display_text=? WHERE custom_emoji_id=?",
                        (display_text, emoji_id),
                    )
                    repaired += 1
                skipped += 1
                continue
        ...
        conn.commit()
    if saved or repaired:
        invalidate_emoji_map_cache()
    return saved, skipped, repaired
...
def _flush_emoji_save_buffer(user_id):
    ...
    skipped = buf["skipped"]
    repaired = buf["repaired"]
    if not saved_items and not skipped and not repaired:
        return
    if not saved_items:
        parts = []
        if skipped:
        ...
        if repaired:
            parts.append(f"{repaired} had a missing premium-mapping and got repaired just now")
        try:
            bot.send_message(chat_id, f"⚠️ No new custom emoji found ({', '.join(parts)}). Send more, or 'done'.")
        except Exception as e:  # noqa: BLE001
    ...
    extra_bits = []
    if skipped:
        extra_bits.append(f"{skipped} already existed")
    if repaired:
        extra_bits.append(f"{repaired} repaired")
    header = f"✅ Saved {n} new emoji" + (f" ({', '.join(extra_bits)})" if extra_bits else "") + \
...
def queue_emoji_save_summary(chat_id, user_id, saved, skipped, repaired=0):
    if not saved and not skipped and not repaired:
        return
    with _emoji_save_buffer_lock:
        buf = _emoji_save_buffers.get(user_id)
        if buf is None:
            buf = {"chat_id": chat_id, "saved": [], "skipped": 0, "repaired": 0, "timer": None}
            _emoji_save_buffers[user_id] = buf
        elif buf["timer"] is not None:
            buf["timer"].cancel()
        buf["saved"].extend(saved)
        buf["skipped"] += skipped
        buf["repaired"] += repaired
...
# ------------------------------------------------------------------
# WELCOME MESSAGE CONTROL: /sroff /sron
# ------------------------------------------------------------------
@bot.message_handler(commands=["sroff"])
def cmd_sroff(message):
    """Disable welcome messages for this group."""
    if message.chat.type not in ("group", "supergroup"):
        bot.reply_to(message, "⚠️ This command works only in groups.")
        return
    
    member = bot.get_chat_member(message.chat.id, message.from_user.id)
    if member.status not in ("administrator", "creator"):
        bot.reply_to(message, "⚠️ Only group admins can use this.")
        return
    
    set_welcome_status(message.chat.id, False)
    bot.reply_to(message, "📵 Welcome messages are now <b>OFF</b> for this group.")


@bot.message_handler(commands=["sron"])
def cmd_sron(message):
    """Enable welcome messages for this group."""
    if message.chat.type not in ("group", "supergroup"):
        bot.reply_to(message, "⚠️ This command works only in groups.")
        return
    ...
    set_welcome_status(message.chat.id, True)
    bot.reply_to(message, "📲 Welcome messages are now <b>ON</b> for this group.")
...
# An autolike row is "active"/pending in one of two mutually exclusive
# ways: an old-style day countdown (mode='days'), or a likes target that
# hasn't been reached yet (mode='likes'). Every place that used to check
# "days_left > 0" to mean "still pending" needs both cases now.
AUTOLIKE_ACTIVE_SQL = "((mode='days' AND days_left>0) OR (mode='likes' AND likes_given_total<target_likes))"


def parse_autolike_target(raw):
    """Parses the 4th /auto (or 3rd /edit) argument.

    "999d" (trailing d/D) -> ("days", 999): run once a day for 999 days.
    "999"  (plain number)  -> ("likes", 999): run daily until this many
        likes have actually been given in total, then auto-complete and
        remove itself — no fixed day count.

    Returns (mode, value) or (None, None) if it doesn't parse.
    """
    raw = raw.strip()
    if raw[-1:].lower() == "d" and raw[:-1].isdigit() and raw[:-1]:
        return "days", int(raw[:-1])
    if raw.isdigit():
        return "likes", int(raw)
    return None, None
...
def cmd_like(message):
    ...
    if not is_service_enabled("like"):
        bot.reply_to(message, "⚠️ Like service is currently disabled. Please try again later.")
        return
...
def cmd_visit(message):
    ...
    if not is_service_enabled("visit"):
        bot.reply_to(message, "⚠️ Visit service is currently disabled. Please try again later.")
        return
...
# ------------------------------------------------------------------
# COMMAND: /auto IND {uid} {target} {name}   — OWNER ONLY (adds)
# target: "7d" = 7 days, "500" = 500-likes target (no trailing d)
# ------------------------------------------------------------------
...
def cmd_auto(message):
    ...
    if len(parts) != 4:
        bot.reply_to(
            message,
            "⚠️ Usage: <code>/auto IND 1234567890 7d MyName</code>\n"
            "(region, uid, target, name)\n\n"
            "Target: <code>7d</code> = runs daily for 7 days.\n"
            "Target: <code>500</code> (no 'd') = runs daily until 500 likes "
            "total have been given, then auto-completes.",
        )
        return
    region, uid, target_raw, name = parts
    region = region.upper()
    if not UID_RE.match(uid):
        bot.reply_to(message, "⚠️ UID must be numeric.")
        return
    mode, target_value = parse_autolike_target(target_raw)
    if mode is None:
        bot.reply_to(
            message,
            "⚠️ Target must be a number of days like <code>7d</code>, or a "
            "plain like-count like <code>500</code>.",
        )
        return

    days = target_value if mode == "days" else 0
    target_likes = target_value if mode == "likes" else 0
    ...
    with _db_lock, db() as conn:
        ...
        if existing:
            # Same UID already has an active auto-like — extend/update
            # that ONE row instead of inserting a duplicate. Duplicate
            # rows for the same account were the actual cause of the
            # same UID showing up twice in the daily results.
            conn.execute(
                "UPDATE autolikes SET days_left=?, mode=?, target_likes=?, likes_given_total=0, "
                "name=?, chat_id=?, added_by=? WHERE id=?",
                (days, mode, target_likes, name, message.chat.id, message.from_user.id, existing["id"]),
            )
        else:
            conn.execute(
                "INSERT INTO autolikes (chat_id, uid, region, name, days_left, mode, target_likes, "
                "likes_given_total, added_by, created_at) VALUES (?,?,?,?,?,?,?,0,?,?)",
                (message.chat.id, uid, region, name, days, mode, target_likes, message.from_user.id,
                 datetime.now(TZ).isoformat()),
            )
        conn.commit()

    h, m = get_setting("autolike_hour", "4"), get_setting("autolike_minute", "0")
    target_display = f"{days} day(s)" if mode == "days" else f"{target_likes} likes (target)"
    text = box(
        "Auto-Like Scheduled ⏰✅",
        [
            ("Name", name), ("UID", uid), ("Region", region), ("Target", target_display),
...
# ------------------------------------------------------------------
# COMMAND: /edit IND {uid} {target} {name}   — OWNER ONLY (updates an existing autolike)
# ------------------------------------------------------------------
...
def cmd_edit(message):
    ...
    if len(parts) != 4:
        bot.reply_to(
            message,
            "⚠️ Usage: <code>/edit IND 1234567890 15d MyName</code>\n"
            "(region, uid, target, name)\n\n"
            "Target: <code>15d</code> switches/extends it to a 15-day autolike.\n"
            "Target: <code>500</code> (no 'd') switches it to a 500-likes target.",
        )
        return
    region, uid, target_raw, name = parts
    region = region.upper()
    ...
    if mode is None:
        bot.reply_to(
            message,
            "⚠️ Target must be a number of days like <code>15d</code>, or a "
            "plain like-count like <code>500</code>.",
        )
        return
    ...
    with _db_lock, db() as conn:
        row = conn.execute(
            "SELECT id FROM autolikes WHERE uid=? AND region=?", (uid, region),
        ).fetchone()
        if not row:
            bot.reply_to(
                message,
                f"⚠️ No auto-like found for UID {uid} ({region}). Use /auto to add one first.",
            )
            return
        conn.execute(
            "UPDATE autolikes SET days_left=?, mode=?, target_likes=?, likes_given_total=0, "
            "name=?, chat_id=?, added_by=? WHERE id=?",
            (days, mode, target_likes, name, message.chat.id, message.from_user.id, row["id"]),
        )
        conn.commit()
...
# ------------------------------------------------------------------
def _merge_double_like(results):
    """results: up to two (data, elapsed, err) tuples from calling the
    like API twice for one UID, IN ORDER (the second call only starts
    after the first one finishes — never fired concurrently against
    the same UID).

    Earlier this ran both calls in PARALLEL and simply summed each
    call's self-reported "given" — but when two requests for the same
    UID land on this kind of unofficial API at nearly the same instant,
    they can both read the same starting count and each report a full
    delta that only actually landed once, or interact in other
    inconsistent ways. The result was a "Given" number that didn't
    match the "Before"/"After" shown right next to it in the same
    message — exactly the mismatch reported. Running the two calls
    strictly one-after-another removes that race, and deriving Given
    as After−Before (rather than trusting each call's own self-reported
    delta) guarantees the three numbers shown always agree, no matter
    what the API's own per-call field says."""
    oks = [r for r in results if r[0] is not None]
    if not oks:
        first_err = results[0][2] if results else None
        return None, None, first_err
    before = oks[0][0].get("LikesbeforeCommand", 0) or 0    # first call's before = the true starting point
    after = oks[-1][0].get("LikesafterCommand", 0) or 0      # last call's after = the true ending point
    data = dict(oks[-1][0])
    data["LikesbeforeCommand"] = before
    data["LikesafterCommand"] = after
    data["LikesGivenByAPI"] = max(after - before, 0)
    elapsed_values = [e for _, e, _ in oks if e is not None]
    elapsed = sum(elapsed_values) if elapsed_values else None
    return data, elapsed, None


def _process_uid_double(row):
    """Sequentially fires the like API twice for this UID (second call
    only after the first completes) and merges the two into one
    internally-consistent result. Submitted as a single task per UID to
    the batch's worker pool, so many UIDs still run fully in parallel —
    only the two calls WITHIN one UID are ordered."""
    results = []
    for _ in range(2):
        try:
            data, elapsed = call_like_api(row["uid"], row["region"])
            results.append((data, elapsed, None))
        except Exception as e:  # noqa: BLE001
            results.append((None, None, e))
    data, elapsed, err = _merge_double_like(results)
    return row, data, elapsed, err


AUTOLIKE_GAP_SECONDS_DEFAULT = 30  # legacy setting, kept for backward compatibility — unused now that
                                    # the batch runs on a shared worker pool instead of one-at-a-time


def autolike_job():
    """Runs the whole batch on a shared pool of parallel workers (size =
    the owner-editable "autolike_workers" setting — this needs to be
    large, since there can be thousands of UIDs) instead of one UID at a
    time. EACH UID gets TWO like-API calls run one after another
    ("double"), merged into one result per UID via _merge_double_like —
    see there for why they run sequentially rather than concurrently.
    The batch can repeat multiple times (owner-editable
    "autolike_repeat_count" setting, default 1). While this runs,
    /like and /visit commands are queued (see _queue_during_autolike)
    instead of running concurrently or being dropped; the moment the
    whole batch finishes, every queued command is replayed in the order
    it arrived.

    Each row is either mode='days' (the original day-countdown, decremented
    once per day regardless of how many repeats ran that day) or
    mode='likes' (keeps running daily, tracking a running total, until
    that many likes have actually landed — then it removes itself
    immediately, even mid-batch, rather than waiting for the day's last
    repeat)."""
    with db() as conn:
        rows = conn.execute(f"SELECT * FROM autolikes WHERE {AUTOLIKE_ACTIVE_SQL}").fetchall()
    if not rows:
        return

    if autolike_in_progress.is_set():
        # Guards against the manual "Restart Auto-Like Now" button (or a
        # misfired second cron trigger) overlapping with a run already in
        # progress — which was the actual cause of UIDs quietly getting
        # processed twice in a "single repeat" setup: two full batches
        # racing on the same rows, each decrementing days_left and
        # queuing/un-queuing /like /visit independently.
        log.warning("autolike_job() called while one is already running — refusing to start a second one.")
        return

    autolike_in_progress.set()
    try:
        workers = int(get_setting("autolike_workers", str(AUTOLIKE_WORKERS)))
        workers = max(1, min(workers, 500))
        repeat_count = int(get_setting("autolike_repeat_count", str(DEFAULT_AUTOLIKE_REPEAT)))
        repeat_count = max(1, min(repeat_count, 10))
        owner_id = get_owner_id()
        all_results = []  # accumulate all results across repeats for owner summary

        row_by_id = {row["id"]: row for row in rows}
        likes_total = {row["id"]: row["likes_given_total"] for row in rows}
        # A likes-target row can hit its target mid-batch, before the
        # day's last repeat — it must stop being sent to the API the
        # moment that happens, not linger until repeat_count.
        active_ids = set(row_by_id.keys())

        for repeat_num in range(1, repeat_count + 1):
            repeat_rows = [row_by_id[i] for i in active_ids]
            if not repeat_rows:
                break
            log.info("Auto-like batch %d/%d starting for %d UIDs x2 calls each, %d parallel workers",
                      repeat_num, repeat_count, len(repeat_rows), workers)
            to_delete, to_decrement, to_persist_likes = [], [], []

            with ThreadPoolExecutor(max_workers=workers) as executor:
                future_to_row = {executor.submit(_process_uid_double, row): row for row in repeat_rows}

                for fut in as_completed(future_to_row):
                    row = future_to_row[fut]
                    rid = row["id"]
                    _, data, elapsed, err = fut.result()
                    chat_id = row["chat_id"]
                    given = (data.get("LikesGivenByAPI", 0) or 0) if data else 0

                    if row["mode"] == "likes":
                        likes_total[rid] = likes_total.get(rid, 0) + given
                        to_persist_likes.append((likes_total[rid], rid))
                    ...
                    if err or data is None:
                        log.error("Auto-like failed uid=%s region=%s repeat=%d: %s",
                                  row["uid"], row["region"], repeat_num, err)
                        try:
                            bot.send_message(
                                chat_id,
                                f"❌ Auto-like failed for <b>{row['name']}</b> (UID {row['uid']}, repeat {repeat_num}/{repeat_count}). Will retry tomorrow.",
                            )
                        except Exception:  # noqa: BLE001
                            pass
                    else:
                        ...
                        if row["mode"] == "days":
                            target_line = f"{row['days_left']} day(s) left"
                        else:
                            target_line = f"{likes_total[rid]}/{row['target_likes']} likes"
                        text = box(
                            f"Auto-Like Result ⏰✅ (Batch {repeat_num}/{repeat_count})",
                            [
                                ("Name", row["name"]), ("Nickname", nickname),
                            ...
                            footer=f"{elapsed:.2f} seconds" if elapsed else "",
                            highlight={"Before", "After", "Given"},
                        )
                        try:
                    ...
                    if row["mode"] == "likes":
                        if likes_total[rid] >= row["target_likes"]:
                            active_ids.discard(rid)
                            to_delete.append(rid)
                            try:
                                bot.send_message(
                                    chat_id,
                                    f"🎯 <b>{row['name']}</b> (UID {row['uid']}) hit its "
                                    f"{row['target_likes']}-like target — auto-like removed.",
                                )
                            except Exception:  # noqa: BLE001
                                pass
                    elif repeat_num == repeat_count:
                        new_days = row["days_left"] - 1
                        if new_days <= 0:
                            to_delete.append(rid)
                        else:
                            to_decrement.append((new_days, rid))

            with _db_lock, db() as conn:
                if to_persist_likes:
                    conn.executemany("UPDATE autolikes SET likes_given_total=? WHERE id=?", to_persist_likes)
                if to_decrement:
                    conn.executemany("UPDATE autolikes SET days_left=? WHERE id=?", to_decrement)
                if to_delete:
                    conn.executemany("DELETE FROM autolikes WHERE id=?", [(i,) for i in set(to_delete)])
                conn.commit()

        # Send summary to owner
        if owner_id and all_results:
            try:
                success_count = sum(1 for r in all_results if not r["err"])
                fail_count = len(all_results) - success_count
                total_given = sum((r["data"].get("LikesGivenByAPI", 0) or 0) for r in all_results if r["data"])
                summary = f"✅ Auto-Like Complete\n\n🔄 Repeats: {repeat_count}\n📊 Total UIDs: {len(set(r['uid'] for r in all_results))}\n"
                summary += f"✅ Success: {success_count}\n❌ Failed: {fail_count}\n❤️ Total Likes: {total_given}"
                bot.send_message(owner_id, summary)
            except Exception as e:  # noqa: BLE001
                log.error("Could not send owner summary: %s", e)

        log.info("Auto-like job finished (repeats=%d, total_calls=%d)", repeat_count, len(all_results))
    finally:
...
BTN_SET_WORKERS = "🧵 Set Auto-Like Workers"
BTN_SET_REPEAT  = "🔄 Set Auto-Like Repeat Count"
BTN_BAN         = "🚫 Ban User"
...
BTN_SERVICE_STATUS = "🔌 Service Status (Like/Visit)"
BTN_AUTO_RESTART = "🔄 Restart Auto-Like Now"
...
def admin_panel_kb():
    ...
    kb.row(colored_button(BTN_SET_GAP, style="primary"), colored_button(BTN_SET_WORKERS, style="primary"))
    kb.row(colored_button(BTN_SET_REPEAT, style="primary"))
    kb.row(colored_button(BTN_SERVICE_STATUS, style="primary"), colored_button(BTN_AUTO_RESTART, style="success"))
...
def back_only_kb():
    """Small keyboard shown while waiting for input in a multi-step
    admin flow, so the owner can always bail out. One-time keyboard
...
def panel_autolikes(message):
    ...
    lines = []
    for r in rows:
        target = f"{r['days_left']}d left" if r["mode"] == "days" else f"{r['likes_given_total']}/{r['target_likes']} likes"
        lines.append(f"• {r['name']} | UID {r['uid']} | {r['region']} | {target} | chat {r['chat_id']}")
...
@bot.message_handler(func=lambda m: m.text == BTN_SET_WORKERS and _owner_dm(m))
def panel_set_workers_start(message):
    admin_state[message.from_user.id] = {"action": "set_autolike_workers"}
    current = get_setting("autolike_workers", str(AUTOLIKE_WORKERS))
    bot.send_message(
        message.chat.id,
        f"🧵 Currently <b>{current} parallel workers</b> during the daily auto-like batch "
        f"(each UID gets 2 like-API calls, run concurrently, using this pool).\n"
        "Send the new worker count (whole number, 1–500), e.g. <code>250</code>.",
        reply_markup=back_only_kb(),
    )


@bot.message_handler(func=lambda m: m.text == BTN_SET_REPEAT and _owner_dm(m))
def panel_set_repeat_start(message):
    admin_state[message.from_user.id] = {"action": "set_autolike_repeat_count"}
    current = get_setting("autolike_repeat_count", str(DEFAULT_AUTOLIKE_REPEAT))
    bot.send_message(
        message.chat.id,
        f"🔄 Currently <b>{current}</b> repeat run(s) of the daily auto-like batch.\n"
        f"Each repeat processes every UID with 2 parallel like-API calls.\n"
        "Send the repeat count (whole number, 1–10), e.g. <code>2</code>.",
        reply_markup=back_only_kb(),
    )
...
@bot.message_handler(func=lambda m: m.text == BTN_SERVICE_STATUS and _owner_dm(m))
def panel_service_status(message):
    """Show service on/off status and allow toggling."""
    like_enabled = is_service_enabled("like")
    visit_enabled = is_service_enabled("visit")
    
    kb = types.InlineKeyboardMarkup()
    like_btn = "✅ Like ON" if like_enabled else "❌ Like OFF"
    visit_btn = "✅ Visit ON" if visit_enabled else "❌ Visit OFF"
    kb.row(types.InlineKeyboardButton(like_btn, callback_data="toggle_service:like"),
           types.InlineKeyboardButton(visit_btn, callback_data="toggle_service:visit"))
    kb.row(types.InlineKeyboardButton("🔙 Back", callback_data="back_to_panel"))
    
    bot.send_message(message.chat.id, 
                     "🔌 <b>Service Status</b>\n\n"
                     f"Like: {'✅ <b>ENABLED</b>' if like_enabled else '❌ <b>DISABLED</b>'}\n"
                     f"Visit: {'✅ <b>ENABLED</b>' if visit_enabled else '❌ <b>DISABLED</b>'}\n\n"
                     "Tap a button to toggle:",
                     reply_markup=kb)


@bot.callback_query_handler(func=lambda c: c.data.startswith("toggle_service:"))
def cb_toggle_service(call):
    service = call.data.split(":")[1]
    current = is_service_enabled(service)
    set_service_status(service, not current)
    ...
    bot.answer_callback_query(call.id, f"{service.upper()} toggled!")


@bot.message_handler(func=lambda m: m.text == BTN_AUTO_RESTART and _owner_dm(m))
def panel_auto_restart(message):
    """Manually restart auto-like batch immediately — runs in the
    background so this button doesn't hang waiting for the whole batch
    (which can take minutes), and refuses to start a second overlapping
    run if one is already in progress (scheduled or manual)."""
    if autolike_in_progress.is_set():
        bot.send_message(message.chat.id, "⚠️ Auto-like is already running right now — wait for it to finish first.")
        return
    with db() as conn:
        pending = conn.execute(f"SELECT COUNT(*) c FROM autolikes WHERE {AUTOLIKE_ACTIVE_SQL}").fetchone()["c"]
    if pending == 0:
        bot.send_message(message.chat.id, "⚠️ No active auto-like UIDs to process.")
        return
    bot.send_message(message.chat.id, f"🔄 Starting auto-like now for {pending} UID(s) — results will come in as they finish.")
    _bg_executor.submit(autolike_job)


@bot.callback_query_handler(func=lambda c: c.data == "back_to_panel")
def cb_back_to_panel(call):
    """Go back to main admin panel (inline callback version)."""
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "🔙 <b>Admin Panel</b>", reply_markup=kb)
    bot.answer_callback_query(call.id)


@bot.message_handler(func=lambda m: _owner_dm(m) and m.text and m.text.strip().lower() in ("🔙 back", "back"))
def handle_back_button_text(message):
    """Handle back button when tapped (text-based approach)."""
    uid = message.from_user.id
    admin_state.pop(uid, None)
    kb = admin_panel_kb()
...
# ------------------------------------------------------------------
_PANEL_BUTTON_TEXTS = {
    BTN_STATS, BTN_AUTOLIST, BTN_RM_AUTO, BTN_SET_TIME, BTN_SET_GAP, BTN_SET_WORKERS, BTN_SET_REPEAT, BTN_SERVICE_STATUS, BTN_AUTO_RESTART, BTN_BAN, BTN_UNBAN, BTN_RESTRICT_UID,
...
def _owner_flow_handler_locked(message, uid):
    ...
    # ---- set auto-like worker pool size (parallel UIDs during the batch) ----
    if action == "set_autolike_workers":
        text = (message.text or "").strip()
        if not text.isdigit() or not (1 <= int(text) <= 500):
            bot.reply_to(message, "⚠️ Send a whole number between 1 and 500, e.g. <code>250</code>.")
            return
        workers = int(text)
        set_setting("autolike_workers", str(workers))
        admin_state.pop(uid, None)
        bot.reply_to(message, f"🧵 Auto-like will now run with <b>{workers} parallel workers</b>.")
        return

    # ---- set auto-like repeat count (how many times batch runs per day) ----
    if action == "set_autolike_repeat_count":
        text = (message.text or "").strip()
        if not text.isdigit() or not (1 <= int(text) <= 10):
            bot.reply_to(message, "⚠️ Send a whole number between 1 and 10, e.g. <code>2</code>.")
            return
        repeat = int(text)
        set_setting("autolike_repeat_count", str(repeat))
        admin_state.pop(uid, None)
        bot.reply_to(message, f"🔄 Auto-like batch will now run <b>{repeat} time(s)</b> each day.")
        return
    ...
    if action == "restore_backup_upload":
        ...
        with _db_lock:
            ...
            # Every already-open pooled connection still has its file
            # handle on the OLD (now-deleted) database — os.replace()
            # doesn't retarget existing handles, it just relinks the
            # path. Without this reset, every handler in the bot kept
            # silently reading/writing the ghost old database and the
            # "restore" never actually took effect until a manual
            # process restart.
            _reset_db_pool()
        init_db()  # make sure any new tables/settings this bot version needs still exist

        # The in-memory premium-emoji map is cached until explicitly
        # invalidated — without this, newly-restored custom_emojis rows
        # never show up until restart, which is why premium emoji looked
        # only "partially" restored.
        invalidate_emoji_map_cache()

        # Clear the last backup message ID so the restored DB doesn't accidentally
        # re-restore from an old pinned message on the next startup
        set_setting("last_backup_message_id", "")
        
        admin_state.pop(uid, None)
        bot.reply_to(message, "✅ Database restored from your uploaded file — live now, no restart needed. "
                              "(Old backups must now be manually deleted from the backup channel if needed.)")
        return
    ...
    if action == "add_emoji_mode":
        ...
        if message.content_type != "text":
            ...
            return
        saved, skipped, repaired = extract_and_save_emojis(message)
        queue_emoji_save_summary(message.chat.id, uid, saved, skipped, repaired)
        return
...
# ------------------------------------------------------------------
# AUTO-COLLECT PREMIUM EMOJIS FROM ALL MESSAGES
# ------------------------------------------------------------------
@bot.message_handler(
    func=lambda m: m.entities is not None and any(e.type == "custom_emoji" for e in m.entities),
    content_types=["text"]
)
def auto_collect_emojis(message):
    """Automatically extract and save premium emojis from any message in the bot."""
    try:
        saved, skipped, repaired = extract_and_save_emojis(message)
        if saved:
            log.info(f"Auto-collected {len(saved)} premium emoji(s) from user {message.from_user.id}")
    except Exception as e:  # noqa: BLE001
        log.error(f"Failed to auto-collect emojis: {e}")
...
if __name__ == "__main__":
    # IMPORTANT: restore must run BEFORE init_db()/any db() call — the
    # very first db() call opens a pooled sqlite3 connection and (via
    # init_db) switches on WAL mode, which creates -wal/-shm sidecar
    # files next to bot_data.db. If that happens first, the backup
    # restore below only overwrites the main file and leaves those
    # sidecar files stale, which is what made restore-on-restart
    # silently fail before. Restoring on a clean slate first, then
    # initializing once, avoids that entirely.
    restore_status = restore_db_from_backup()
    init_db()
    ...
    # Ping every owner on every startup/redeploy so a Railway redeploy,
    # crash-restart, or manual restart is always visible in the bot
    # itself — not just in logs nobody is watching.
    deploy_notice = (
        "🚀 <b>Latest deploy detected</b>\n"
        f"🗄 {restore_status}"
    )
    for oid in OWNER_IDS:
        try:
            bot.send_message(oid, deploy_notice)
        except Exception as e:  # noqa: BLE001
            log.warning("Could not send deploy notice to owner %s: %s", oid, e)

    # NOTE: bot.infinity_polling() catches every exception INSIDE itself
    # and retries forever on its own with a short fixed sleep — it never
    # raises back out to this try/except, even for a 409 "Conflict:
    # terminated by other getUpdates request" error, which Telegram sends
    # whenever a second process is polling with the same token at the
    # same time. Using bot.polling() directly instead lets that error
    # actually reach this except block so we can at least LOG it clearly
    # and alert the owner — rather than it being silently swallowed and
    # showing up only as unexplained flaky behavior.
    #
    # IMPORTANT: this does NOT exit the process on a conflict. An earlier
    # version of this code did exit on 409, on the assumption that a
    # conflict only ever means a brief redeploy overlap that resolves
    # itself once the old container finishes shutting down. But if
    # Railway is actually configured with more than one persistent
    # replica (or the bot is deployed twice by mistake), a conflict never
    # goes away — exiting just makes Railway immediately restart the
    # container, which conflicts again instantly, which exits again...
    # an infinite crash-restart loop, which is worse than the original
    # problem (and breaks in-progress things like the "Restore From
    # File" flow every few seconds). Simply retrying, forever, is the
    # safe default. Actual data-loss from a duplicate instance's own
    # backup cycle is already prevented separately by the row-count
    # guard in backup_db_job() above — that guard is what actually
    # matters here, not killing the process.
    consecutive_conflicts = 0
    last_conflict_alert = 0
    while True:
        try:
            bot.polling(non_stop=False, skip_pending=True, timeout=30, long_polling_timeout=30)
        except telebot.apihelper.ApiTelegramException as e:
            if getattr(e, "error_code", None) == 409:
                consecutive_conflicts += 1
                log.error(
                    "409 Conflict (%d in a row): another instance of this bot is "
                    "already polling with this same token — NOT exiting, just "
                    "retrying. If this never stops, check Railway for more than "
                    "one running instance/replica of this service.",
                    consecutive_conflicts,
                )
                # Only nag the owner once per 10 minutes so this doesn't spam
                # them the way the crash-loop itself used to.
                now = time.time()
                if consecutive_conflicts >= 5 and now - last_conflict_alert > 600:
                    last_conflict_alert = now
                    for oid in OWNER_IDS:
                        try:
                            bot.send_message(
                                oid,
                                "⚠️ This bot instance keeps getting a 409 Conflict "
                                "error — another copy of the bot is polling with "
                                "the same token at the same time. Please check "
                                "Railway for duplicate/extra running instances of "
                                "this service.",
                            )
                        except Exception:  # noqa: BLE001
                            pass
